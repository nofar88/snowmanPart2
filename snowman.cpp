...
string ariel::snowman(int a){
    ...
    //Checking that the input we received for the snowman is within range
    if(a>MAX || a< MIN) {
    	throw std::range_error("out of bound \n");
    }

    //Converts the input we got for the snowman to a string so we can go through an index
